...
lista_cidades = ["ESTADUAL", "FLORIANÓPOLIS", "ITAJAÍ", "JOINVILLE", "CHAPECÓ"]
...
focos["ESTADUAL"] = focos.drop(columns = "Semana").sum(axis=1)
casos["ESTADUAL"] = casos.drop(columns = "Semana").sum(axis=1)
tmin["ESTADUAL"] = tmin.drop(columns = "Semana").mean(axis=1)
...
class Descritiva:
	...
	def __init__(self, csv):
		...
		"""
		print("\n" + "="*80 + "\n")
		print(f"\n{ansi['cyan']}>>>OBJETO ESTATÍTICO DESCRITIVO INSTANCIADO<<<{ansi['reset']}")
		print("\n" + "="*80 + "\n")
		self.csv = csv
		print(csv)
		print("\n" + "="*80 + "\n")
	...
	def __getitem__(self, chave):
		return self.csv[chave]

	def __str__(self):
		return self.csv		

	### Definindo Função
	def normalizando_nome_cidade(self, cidade):
		"""
	...
	def visualiza_focos(self, cidade, focos):
	...
	def histograma(self, cidade, csv, str_var):
		"""
	...
	def decomposicao_sazonal(self, cidade, csv, str_var):
		"""
...
focos = Descritiva(focos)
casos = Descritiva(casos)
prec = Descritiva(prec)
tmin = Descritiva(tmin)
tmed = Descritiva(tmed)
tmax = Descritiva(tmax)

for i in lista_cidades:

	focos.histograma(i, focos, "focos")
	casos.histograma(i, casos, "casos")
	prec.histograma(i, prec, "prec")
	tmin.histograma(i, tmin, "tmin")
	tmed.histograma(i, tmed, "tmed")
	tmax.histograma(i, tmax, "tmax")

	focos.decomposicao_sazonal(i, focos, "focos")
	casos.decomposicao_sazonal(i, casos, "casos")
