...
\begin{figure}
    ...
    \includegraphics[width=0.5\linewidth]{figuras/area_de_estudo_SC_MATHEUS.png}
\end{figure}
...
\indent O comportamento da precipitação é menos suave aos dados agrupados semanalmente, diferente das temperaturas. Porém, assim como as temperaturas, esse comportamento é por conta do método de agrupamento, baseado no somatório dos dias da semana. Logo, os valores semanais de precipitação são maiores, sejam eles médio, máximo ou desvio padrão.

\indent O valor mínimo diário e semanal é semelhante, zero (0) mm, pois há períodos em que não precipita em \acrlong{SC} por mais de sete (7) dias. Esse valor também diz respeito a característica do dado, sendo uma variável quantitativa de razão, em contraposição às temperaturas, que são variáveis quantitativas intervalares.

\indent \textcolor{red}{Interessante Amplitude Térmica? (Debater com o Mário)\\
Como aplicar Amplitude a esses dados?}

\indent A distribuição sazonal de todas as variáveis seguiu cronologicamente as semanas epidemiológicas, dessa maneira há concordância temporal entre os elementos climatológicos (precipitação e temperaturas mínima, média e máxima) e variáveis entomológicas (focos de \latim{Aedes} sp.) e epidemiológicas (casos de dengue).

\indent Também foi realizada a distribuição sazonal total, com a média das semanas epidemiológicas de todas as variáveis (figura \ref{fig: distribuicao_sazonal_SC_total}). Percebe-se que tanto para focos de \latim{Aedes} sp. quanto para casos de dengue há concentrações sazonais regionalizadas, principalmente: Oeste Catarinense, nas proximidades dos municípios de Chapecó e Concórdia; Norte Catarinense, com especial destaque para o município de Joinville; Vale do Itajaí, nas proximidades dos municípios de Itajaí e Blumenau; e Grande Florianópolis.

\indent Por esses comportamentos regionalizados na sazonalidade, as quatro (4) cidades elencadas nos estudos foram: Florianópolis, Itajaí, Joinville e Chapecó. Também po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r apresentarem frequência e volume de dados, além de representarem municípios populosos e distintos.

\begin{landscape}
\begin{figure}[htbp]
    \begin{center}
    \caption{Distribuições de sazonalidade de variváveis entomo-epidemiológicas (focos de \latim{Aedes} sp. e casos de dengue) e climatológicas (precipitação e temperaturas mínima, média e máxima) em Santa Catarina.}
    \label{fig: distribuicao_sazonal_SC_total}
    \subfloat[Casos de Dengue \label{fig: casos_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }
    \subfloat[Focos de \latim{Aedes} sp. \label{fig: focos_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/focos_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }
    \subfloat[Precipitação (mm) \label{fig: prec_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/prec_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }\hfill
    \subfloat[Temperatura Mínima (C) \label{fig: tmin_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/tmin_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }
    \subfloat[Temperatura Média (C) \label{fig: tmed_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/tmed_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }
    \subfloat[Temperatura Máxima (C) \label{fig: tmax_mapa_coropletico_sazonal_se_total}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/tmax_mapa_coropletico_sazonal_se_total_dissertacao_compressed.pdf}
        }\hfill
    \end{center}
    \small{Fonte: Elaboração própria (2024).}
\end{figure}
...
\indent É possível perceber, em primeira análise (figura \ref{fig: distribuicao_sazonal}), que os picos de casos de dengue são atingidos algumas semanas epidemiológicas após os valores máximos de ambas as temperaturas, apresentando um hiato de 10 a 15 semanas entre esses picos.

\indent Esses máximos valores de casos de dengue também precedem os mínimos registros de ambas as temperaturas, com hiato entre cinco e 10 semanas epidemiológicas. O que corrobora com o comportamento sazonal da doença, ocorrendo com maior incidência durante as semanas epidemiológias do outono austral. Ainda durante o inverno no hemisfério sul, pode-se perceber leve queda nos focos de \latim{Aedes} sp.

\begin{figure}[htbp]
    \begin{center}
    \caption{Distribuições de sazonalidade de variáveis entomo-epidemiológicas (focos de \latim{Aedes} sp. e casos de dengue) e climatológicas (precipitação e temperaturas mínima, média e máxima) em alguns municípios catarinenses.}
    \label{fig: distribuicao_sazonal}
    \subfloat[Florianópolis \label{fig: distribuicao_sazonal_FLO}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/distribuicao_sazonalidade_semanal_FLORIANOPOLIS.pdf}
        }
    \subfloat[Itajaí \label{fig: distribuicao_sazonal_ITA}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/distribuicao_sazonalidade_semanal_ITAJAI.pdf}
        }\hfill
    \subfloat[Joinville \label{fig: distribuicao_sazonal_JOI}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/distribuicao_sazonalidade_semanal_JOINVILLE.pdf}
        }
    \subfloat[Chapecó \label{fig: distribuicao_sazonal_CHA}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/distribuicao_sazonalidade_semanal_CHAPECO.pdf}
        }\hfill
    \end{center}
    \small{Fonte: Elaboração própria (2024).}
\end{figure}

\indent Pela de ocorrência de inflexão nas curvas de ambas as temperaturas coincidir com o aumento dos casos de dengue e por apresentar hiatos entre os picos, selecionou-se semanas epidemiológicas, especialmente entre a décima primeira (11ª) e a vigésima segunda (22ª) semanas. As sazonalidades apresentadas... (figura \ref{fig: distribuicao_sazonal_SC}) ...to be continued...

\begin{figure}[htbp]
    \begin{center}
    \caption{Distribuições de sazonalidade de casos de dengue em Santa  durante algumas semanas epidemiológicas.}
    \label{fig: distribuicao_sazonal_SC}
    % \subfloat[Semana Epidemiológica: 11 \label{fig: casos_mapa_coropletico_sazonal_se11}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se11.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 12 \label{fig: casos_mapa_coropletico_sazonal_se12}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se12.pdf}
        }\hfill
    % \subfloat[Semana Epidemiológica: 13 \label{fig: casos_mapa_coropletico_sazonal_se13}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se13.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 14 \label{fig: casos_mapa_coropletico_sazonal_se14}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se14.pdf}
        }\hfill
    % \subfloat[Semana Epidemiológica: 15 \label{fig: casos_mapa_coropletico_sazonal_se15}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se15.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 16 \label{fig: casos_mapa_coropletico_sazonal_se16}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se16.pdf}
        }\hfill
    % \subfloat[Semana Epidemiológica: 17 \label{fig: casos_mapa_coropletico_sazonal_se17}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se17.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 18 \label{fig: casos_mapa_coropletico_sazonal_se18}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se18.pdf}
        }\hfill
    % \subfloat[Semana Epidemiológica: 19 \label{fig: casos_mapa_coropletico_sazonal_se19}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se19.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 20 \label{fig: casos_mapa_coropletico_sazonal_se20}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se20.pdf}
        }\hfill
    % \subfloat[Semana Epidemiológica: 21 \label{fig: casos_mapa_coropletico_sazonal_se21}]{
    %     \centering
    %     \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se21.pdf}
    %     }
    \subfloat[Semana Epidemiológica: 22 \label{fig: casos_mapa_coropletico_sazonal_se22}]{
        \centering
        \includegraphics[width=0.47\textwidth]{figuras/casos_mapa_coropletico_sazonal_se22.pdf}
        }\hfill
    \end{center}
    \small{Fonte: Elaboração própria (2024).}
\end{figure}
...
2023-12-31 & 1 & SÃO CRISTOVÃO DO SUL \\
