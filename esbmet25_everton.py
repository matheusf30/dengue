...
distritos = "floripa_distritos.shp"


### Abrindo Arquivo
#casos = pd.read_csv(f"{caminho_dados}{casos}", low_memory = False)
#focos = pd.read_csv(f"{caminho_dados}{focos}", low_memory = False)
distritos = gpd.read_file(f"{caminho_shp}{distritos}")


#print(f"\n{green}CASOS\n{reset}{casos}\n")
#print(f"\n{green}FOCOS\n{reset}{focos}\n")

print(f"\n{green}GEODATAFRAME DOS DISTRITOS DE FLORIANÓPOLIS\n{reset}{distritos}\n")
distritos.plot()
plt.show()
...
def cartografia_sazonal_entomoepidemio_total(csv_melt, str_var):
	...
	distritos.plot(ax = ax, color = "lightgray", edgecolor = "lightgray")
...
for semana_epidemio in range(18,23):
	print(f"\n{green}SEMANA EPIDEMIOLÓGICA: {semana_epidemio}{reset}\n")
...
for semana_epidemio in range(50,54):
	try:
		print(f"\n{green}SEMANA EPIDEMIOLÓGICA: {semana_epidemio}{reset}\n")
		cartografia_sazonal_entomoepidemio(casos, "casos", semana_epidemio)
		print(f"\n{green}SEMANA EPIDEMIOLÓGICA: {semana_epidemio}{reset}\n")
